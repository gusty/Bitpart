...
type Protocol () = 
    ...
    override appServer.OnStartup () =
        ...
        let send message = function            
            | null -> log Debug "Session does not exists anymore (sender = %s, rcpts = %A, subject = %s)." message.sender message.recipients message.subject
        ...
        let DetectDisconnect (session: CustomProtocolSession) (reason: CloseReason) users = function
            | Some sender -> log Info "- Session: %A%-26s disconnected from Movie: %-22s. Reason: %A" session sender.Name sender.Movie reason; removeUser sender users
            | None        -> log Info "? Session: %A disconnected before login. Reason: %A"           session                          reason; users
            ...
            | None -> 
                let users = 
                    ...
                    | Some existingUser ->                        
                        ...
                        log Info "? Session: %A User already exists in that movie. Disconnect existing user:%s from movie:%s, session %s" session userId movieId existingUser.Session
                ...
                log Info "+ Session: %A %-26s connected to      Movie: %-22s (Protocol:%i.%i Client:%i.%i)" session userId movieId <|| session.ProtocolVersion <|| session.ClientVersion
        ...
        let processAdminCommand session cmd plist =
            ...
            try
                ...
                | "s" -> 
                    ...
                    reply (sprintf "Peak Users   : = %i at %s" <|| second formatDateTime appServer.MaxUserCount   )
                    reply (sprintf "Peak Sessions: = %i at %s" <|| second formatDateTime appServer.MaxSessionCount)
        ...
        let evtRequest (session :CustomProtocolSession) (requestInfo :Protocol.BinaryRequestInfo) =  
            ...
            with exn -> 
                log Warn "Process message failed for session: %A error: %A" session exn
                session.Close CloseReason.ProtocolError

        let evtLogin (session: CustomProtocolSession) =
            if isInBlackList session then
                log Warn "Session %A is Blacklisted, will be disconnected." session

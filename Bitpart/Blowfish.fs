...
module Blowfish =  
    ...
    let inPlaceInt64ToBytes (lValue:int64) (buffer:byte[]) offset =
        ...
        buffer.[offset + 3] <- byte (lValue >>> 32)
        buffer.[offset + 4] <- byte (lValue >>> 24)
    ...
    let [<Literal>] PBOX_ENTRIES = 18
    let [<Literal>] SBOX_ENTRIES = 256

    let decryptBlock (pbox:uint32[],sbox1:uint32[],sbox2:uint32[],sbox3:uint32[],sbox4:uint32[]) (lCipherBlock:int64) =

        let mutable nHi = uint32 (lCipherBlock >>> 32)
        let mutable nLo = uint32  lCipherBlock

        let f x = ((sbox1.[int (x >>> 24)] + sbox2.[int ((x >>> 16) &&& 0x0ffu)]) ^^^ sbox3.[int ((x >>> 8) &&& 0x0ffu)]) + sbox4.[int (x &&& 0x0ffu)]

        nHi <- nHi           ^^^ pbox.[17]
        nLo <- nLo ^^^ f nHi ^^^ pbox.[16]
        nHi <- nHi ^^^ f nLo ^^^ pbox.[15]
        nLo <- nLo ^^^ f nHi ^^^ pbox.[14]
        nHi <- nHi ^^^ f nLo ^^^ pbox.[13]
        nLo <- nLo ^^^ f nHi ^^^ pbox.[12]
        nHi <- nHi ^^^ f nLo ^^^ pbox.[11]
        nLo <- nLo ^^^ f nHi ^^^ pbox.[10]
        nHi <- nHi ^^^ f nLo ^^^ pbox.[9]
        nLo <- nLo ^^^ f nHi ^^^ pbox.[8]
        nHi <- nHi ^^^ f nLo ^^^ pbox.[7]
        nLo <- nLo ^^^ f nHi ^^^ pbox.[6]
        nHi <- nHi ^^^ f nLo ^^^ pbox.[5]
        nLo <- nLo ^^^ f nHi ^^^ pbox.[4]
        nHi <- nHi ^^^ f nLo ^^^ pbox.[3]
        nLo <- nLo ^^^ f nHi ^^^ pbox.[2]
        nHi <- nHi ^^^ f nLo
    ...
    let blowfishCypher (bfkey:byte[]) =
        ...
        let _sbox3 = Array.init SBOX_ENTRIES (fun i -> S.[2, i])
        ...
        let f (arr:uint32[]) n zero =
            ...
            for nI in 0..2..n-1 do
                _zero <- decryptBlock (_pbox, _sbox1, _sbox2, _sbox3, _sbox4) _zero
                arr.[nI    ] <- uint32 (_zero >>> 32)
                arr.[nI + 1] <- uint32 (_zero &&& -1L)
            _zero

        0L
        |> f _pbox  PBOX_ENTRIES 
        |> f _sbox1 SBOX_ENTRIES 
        |> f _sbox2 SBOX_ENTRIES 
        |> f _sbox3 SBOX_ENTRIES 
        |> f _sbox4 SBOX_ENTRIES
        |> ignore

        _pbox, _sbox1, _sbox2, _sbox3, _sbox4


    let encrypt_decrypt (pbox, sbox1, sbox2, sbox3, sbox4) (buffer:byte[]) =
